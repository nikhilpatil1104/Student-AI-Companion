...
import {
  ...
  Zap, ChevronDown, ChevronUp, CheckCircle, ListTodo, Clock
} from 'lucide-react';

const SECTION_CONFIG = {
  academic:  { icon: BookOpen,   label: 'Academic Impact',   color: '#4FC3F7', bg: 'rgba(79,195,247,0.08)',  border: 'rgba(79,195,247,0.2)'  },
  financial: { icon: DollarSign, label: 'Financial Impact',  color: '#00F5C4', bg: 'rgba(0,245,196,0.08)',   border: 'rgba(0,245,196,0.2)'   },
  stress:    { icon: Heart,      label: 'Stress & Wellness', color: '#FF6B6B', bg: 'rgba(255,107,107,0.08)', border: 'rgba(255,107,107,0.2)' },
};
...
function ImpactSection({ type, data, delay }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden response-section"
      style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, animationDelay: `${delay}ms` }}>
        ...
        <div className="flex items-center gap-2">
          {delta !== 0 && (
            <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-full"
              style={{ color: delta > 0 ? '#00F5C4' : '#FF6B6B', background: delta > 0 ? 'rgba(0,245,196,0.15)' : 'rgba(255,107,107,0.15)' }}>
              {delta > 0 ? `+${delta}` : delta}
            </span>
          )}
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-2">
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl text-sm" style={{ background: 'rgba(255,255,255,0.04)' }}>
...
function TodaysTaskSection({ tasks, delay }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden response-section"
      style={{ background: 'rgba(108,99,255,0.07)', border: '1px solid rgba(108,99,255,0.22)', animationDelay: `${delay}ms` }}>
      <button className="w-full flex items-center justify-between p-4 text-left"
        onClick={() => setExpanded(e => !e)}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(108,99,255,0.2)' }}>
            <ListTodo size={16} color="#6C63FF" />
          </div>
          <span className="font-display font-semibold text-sm text-white/90">Today's Task Plan</span>
            ...
            return (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl"
                ...
                <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                  ...
                  <span className="text-xs font-mono text-white/40 whitespace-nowrap">{t.time}</span>
                </div>
                ...
                <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-full flex-shrink-0 mt-0.5"
...
export default function AIResponsePanel({ response, isLoading, query }) {
  ...
  if (!response) {
    return (
      <div className="glass-card p-8 flex flex-col items-center justify-center text-center gap-4" style={{ minHeight: 280 }}>
        ...
        <div>
          <h3 className="font-display font-bold text-lg text-white/80 mb-1">What's on your mind?</h3>
          <p className="text-sm text-white/30 max-w-xs leading-relaxed">
            Ask me anything about your day — studies, money, stress, productivity.
          </p>
        </div>
        <div className="flex flex-wrap justify-center gap-2 mt-2">
          {["Should I skip class today?", "How do I stick to my budget?", "I have 3 exams this week"].map(hint => (
    ...
    <div className="space-y-3">
      ...
      <div className="glass-card p-5 response-section" style={{ animationDelay: '0ms' }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(108,99,255,0.2)' }}>
      ...
      {/* Recommendation */}
      <div className="rounded-2xl p-5 response-section"
        style={{ background: 'linear-gradient(135deg,rgba(108,99,255,0.12),rgba(0,245,196,0.06))', border: '1px solid rgba(108,99,255,0.25)', animationDelay: '400ms' }}>
        <div className="flex items-center gap-2 mb-2">
          <Lightbulb size={16} color="#6C63FF" />
          <span className="font-display font-semibold text-sm" style={{ color: '#6C63FF' }}>Today's Recommendation</span>
        </div>
      ...
      {response.musicSuggestion && (
        <div className="rounded-2xl p-4 response-section flex items-start gap-4"
          style={{ background: 'rgba(0,245,196,0.06)', border: '1px solid rgba(0,245,196,0.15)', animationDelay: '600ms' }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(0,245,196,0.15)' }}>
            <Music size={18} color="#00F5C4" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-display font-semibold text-sm" style={{ color: '#00F5C4' }}>Music for Your Mood</span>
              <span className="text-xs px-2 py-0.5 rounded-full font-mono" style={{ background: 'rgba(0,245,196,0.15)', color: '#00F5C4' }}>
      ...
      {/* Motivation */}
      {response.motivation && (
        <div className="rounded-2xl p-4 response-section text-center"
          style={{ background: 'rgba(255,215,0,0.05)', border: '1px solid rgba(255,215,0,0.12)', animationDelay: '700ms' }}>
          <p className="text-sm text-white/70 italic leading-relaxed">✨ "{response.motivation}"</p>
        </div>
      )}
    </div>
  );
